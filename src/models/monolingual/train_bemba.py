"""
Train a monolingual Wav2Vec2 CTC model on Bemba.

Usage:
    python -m src.models.monolingual.train_bemba \\
        [--config configs/bemba_config.yaml] \\
        [--output_dir outputs/bemba] \\
        [--dataset_path outputs/bemba/dataset] \\
        [--vocab_path outputs/bemba/vocab.json]
...
DEFAULT_CONFIG = "configs/bemba_config.yaml"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train monolingual Wav2Vec2 CTC model on Bemba."
    )
    parser.add_argument(
        "--config",
        default=DEFAULT_CONFIG,
        help="Path to the Bemba YAML config file.",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
...
def main() -> None:
    ...
    output_dir = args.output_dir or config.get("output_dir", "outputs/bemba")
    ...
    logger.info("=== Bemba Monolingual ASR Training ===")
