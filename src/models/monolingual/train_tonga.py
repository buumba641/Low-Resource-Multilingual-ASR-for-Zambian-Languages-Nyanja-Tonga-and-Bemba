"""
Train a monolingual Wav2Vec2 CTC model on Tonga.

Usage:
    python -m src.models.monolingual.train_tonga \\
        [--config configs/tonga_config.yaml] \\
        [--output_dir outputs/tonga] \\
        [--dataset_path outputs/tonga/dataset] \\
        [--vocab_path outputs/tonga/vocab.json]
...
DEFAULT_CONFIG = "configs/tonga_config.yaml"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train monolingual Wav2Vec2 CTC model on Tonga."
    )
    parser.add_argument(
        "--config",
        default=DEFAULT_CONFIG,
        help="Path to the Tonga YAML config file.",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
...
def main() -> None:
    ...
    output_dir = args.output_dir or config.get("output_dir", "outputs/tonga")
    ...
    logger.info("=== Tonga Monolingual ASR Training ===")
